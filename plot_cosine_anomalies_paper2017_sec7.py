...
Aratio=np.array([]); flength=np.array([]); flength2=np.array([])
fig, ax=plt.subplots()
Dx=500.
...
flength = np.append(flength, (XArr-Dx)[((AmpArr/amp*amp2/AmpArr2)[(XArr-Dx)> 3000.]).max() == (AmpArr/amp*amp2/AmpArr2) ] - 3000.)
...
A_q[D>800] = A_q[D==800]
A_q[D<0]=1.
ax.plot(XArr-Dx, A_q,'g-', lw=3, markersize=10, label='Anelastic' )
